...
| `[✓]` | P2 | APT-137 | New Overall Rating bands per client table — **9/9 ⭐ Exceeds · 6–8/9 ✓ Meets · 3–5/9 ⚠ Needs Improvement · 0–2/9 ✗ Below** (was: 100% Meets / 75–99% Needs Improvement / <75% Below, with Exceeds never reachable). Bands are applied proportionally, so they still hold when fewer than 9 metrics are scored (e.g. 4/6 = Meets). Icons now shown beside the rating; rating colour now follows the computed rating instead of the stale database label. Recalculates immediately after editing custom goals (APT-88). **Test:** Agent Performance → an agent with all metrics on track = Exceeds; adjust a custom goal so a metric flips → rating and colour update at once. | Agent Perf / Team | — | 21-Jul |

...
|--------|-------|-----|------|------|----------|-----|
| `[✅]` | P2 | APT-136 | On/Off Track disagreed with the numbers on screen — e.g. Tracy July "% Achieved First Reply SLA" showed **95% vs 95%** but read Off Track, because the raw value (94.6) was compared while the display rounds to whole numbers (APT-129). Status is now judged on the same rounded figures that are shown, so the badge always matches what you can see. Applies to Agent Performance, Team Dashboard and the attendance overlay. Hovering a value shows the exact underlying number. **Test:** Tracy → July → that row should now read On Track; spot-check other rows still read correctly. | Agent Perf / Team | — | 21-Jul |
| `[✅]` | P2 | APT-137 | New Overall Rating bands per client table — **9/9 ⭐ Exceeds · 6–8/9 ✓ Meets · 3–5/9 ⚠ Needs Improvement · 0–2/9 ✗ Below** (was: 100% Meets / 75–99% Needs Improvement / <75% Below, with Exceeds never reachable). Bands are applied proportionally, so they still hold when fewer than 9 metrics are scored (e.g. 4/6 = Meets). Icons now shown beside the rating; rating colour now follows the computed rating instead of the stale database label. Recalculates immediately after editing custom goals (APT-88). **Effective from July 2026 only** — June 2026 and earlier keep the original bands so historical reviews are not rewritten (cutoff set by `NEW_RATING_BANDS_FROM` in `src/lib/format.js`). **Test:** Agent Performance → an agent with all metrics on track = Exceeds; adjust a custom goal so a metric flips → rating and colour update at once. Switch the same agent to June 2026 → old bands apply (no Exceeds; 8/9 reads Needs Improvement). | Agent Perf / Team | — | 21-Jul |
| `[✅]` | P2 | APT-138 | Follow-up to APT-136 — some rows still showed Off Track when the numbers matched (e.g. Aug 2026 "% Solved in 1 Day" **55% vs 55%** = Off Track). APT-136 only fixed rows the app recalculates (custom goals / historical goals); every other row kept the status the database had worked out from unrounded values. The app now decides On/Off Track for **every** metric row on both Agent Performance and the Team Dashboard, so the badge always matches the figures shown. **Test:** Aug 2026 → "% Solved in 1 Day" should read On Track; check rows genuinely below goal (Articles Created 0 vs 2, Overall SLA 74 vs 80) still read Off Track, and that On/Off Track counts and the Overall Rating update to match. | Agent Perf / Team | — | 17-Sep |
